...
class UserInterface():
    ...
    def __init__(self):
        ...
        #Additions for selecting plot type for user data
        ...
        self.multiplot_keys_button = pn.widgets.Button(styles={}, margin=(23, 0, 0, 0))
        self.multiplot_plot_variable_dropdown = pn.widgets.Select()
        self.multiplot_x_axis_dropdown = pn.widgets.Select()
        self.multiplot_select_variable_button = pn.widgets.Button(name='Plot Loaded Datasets', button_type='success', margin=(23, 0, 0, 10))
        self.multiplot_keys_update_button = pn.widgets.Button(name='Update Loaded Dataset', button_type='primary', margin=(23, 0, 0, 10))
    ...
    def _display_multiplot_user_data_selection_ui(self):
        ...
        self.multiplot_ref_keys_selection_row.append(self.multiplot_select_variable_button)
    ...
    def _multiplot_plot_data_button_click(self):
        ...
        overlay_fig = self._plot_multiplot_dataset(self.multiplot_plot_variable_dropdown.value, x_axis)
    
        # Create a new pane for the figure
        new_plot_pane = pn.pane.Matplotlib(overlay_fig, tight=True)
    ...
    def _multiplot_ref_keys_dropdown_click(self):
        ...
        if self.multiplot_keys_dropdown.value in list(selected_ref_model_cat.keys()) and not self.multiplot_ref_keys_dropdown.value in self.multiplot_ref_dataset_dict:
            ...
            self._update_multiplot_warning_text("Overlay Plot Status >> There is no dataset matching the user dataset in this model, please select another")
    ...
    def _display_multiplot_plot_choices_ui(self):
        ...
        if show_plot_choices:    
            # Insert the UI row under the variable selection even if there are plots already
            if hasattr(self, 'multiplot_ref_keys_selection_row') and self.multiplot_ref_keys_selection_row in self.widget_container:
                insert_index = self.widget_container.index(self.multiplot_ref_keys_selection_row) + 1
    ...
    def _check_multiplot_plot_validity(self):
        ...
        for dataset in list(self.multiplot_ref_dataset_dict.keys()):
            if self.multiplot_plot_variable_dropdown.value not in self.multiplot_ref_dataset_dict[dataset]:
        ...
        if invalid_datasets:
            ...
            self._update_multiplot_warning_text(f"Warning >> The following models were removed as they do not contain the selected variable: {invalid_str}")
    ...
    def _plot_multiplot_dataset(self, variable, x_axis):
        ...
        if self.prompt_bounds_dropdown.value == "Constrain to user dataset bounds":
            ...
            self.prompt_bounds_dropdown.value = "Constrain to min-max reference dataset bounds"
        else:
        ...
        ax.set_xlim(x_min, x_max) #set x-limits based on user choice
        fig.tight_layout()
        ax.set_title(title_text, fontsize=14)
        ...
        for ref_ds in self.multiplot_ref_dataset_dict.values():
            if x_axis in ref_ds:
                ...
                try:
                    ...
                    g_min_tup = (g_min_obj.year, g_min_obj.month, g_min_obj.day, g_min_obj.hour, g_min_obj.minute, g_min_obj.second)
                    r_min_tup = (r_min_obj.year, r_min_obj.month, r_min_obj.day, r_min_obj.hour, r_min_obj.minute, r_min_obj.second)
                    
                    g_max_tup = (g_max_obj.year, g_max_obj.month, g_max_obj.day, g_max_obj.hour, g_max_obj.minute, g_max_obj.second)
                    r_max_tup = (r_max_obj.year, r_max_obj.month, r_max_obj.day, r_max_obj.hour, r_max_obj.minute, r_max_obj.second)
